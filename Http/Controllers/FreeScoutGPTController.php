...
class FreeScoutGPTController extends Controller
{
    ...
    public function getAvailableModels(Request $request)
    {
        ...
        $isDirectOpenAI = empty($baseUrl) || strpos($baseUrl, 'api.openai.com') !== false;

        if ($isDirectOpenAI && !$apiKey) {
            return response()->json(['error' => 'API key is required'], 400);
        }

        $effectiveBaseUrl = rtrim($baseUrl ?: 'https://api.openai.com', '/');
        $effectiveBaseUrl = preg_replace('#/v1$#', '', $effectiveBaseUrl);
        $cacheKey = 'gateway_models_' . md5($effectiveBaseUrl . '_' . ($apiKey ?? ''));
        ...
        try {
            ...
            $response = $client->get($effectiveBaseUrl . '/v1/models', [
                'headers' => $headers,
            ]);
            ...
            if ($isDirectOpenAI) {
                $modelList = array_filter($modelList, function ($model) {
    ...
    public function generate(Request $request)
    {
        ...
        $baseUrl = rtrim($settings->api_base_url ?? '', '/') ?: 'https://api.openai.com';
        ...
        $tokenLimit = (int) $settings->token_limit;
        $userQuery = $request->get('query');
        $isDirectOpenAI = empty($settings->api_base_url) || strpos($settings->api_base_url, 'api.openai.com') !== false;
        ...
        if (!empty($settings->use_responses_api)) {
            ...
            try {
                ...
                $response = $guzzle->post($baseUrl . '/v1/responses', [
                    'headers' => $headers,
                    'body' => json_encode($payload),
                ]);
                $data = json_decode($response->getBody(), true);
                ...
                if (method_exists($e, 'getResponse') && $e->getResponse()) {
                    ...
                    if (isset($json['error']['message'])) {
                        $gatewayError = $json['error']['message'];
                    } else {
                        $gatewayError = $body;
                    }
                }
                $answerText = $gatewayError ?: $errorMsg;
            ...
            try {
                ...
                $answerText = $gatewayError ?: $errorMsg;
                \Log::error('Error on Chat Completions Request: ' . $answerText);
                return Response::json([
                    'query' => $userQuery ?? '',
                    'answer' => $answerText
                ], 200);
